...
import Supercluster from '../../../util/superCluster';
export default {
  name: 'ol-gx-cluster',
  components: {},
  mounted() {
    ...
    mapInit() {
      ...
      fetch('/data/TEM.json').then(response => {
        response.json().then(res => {
          const data = res.data;
          const _data = this.toGeoJson(data);
          const cluster = this.getCluster(
            _data,
            [103.858427734375, 21.2660107421875, 113.64724609375, 27.2535595703125],
            7
          );

          this.contourInit(map, cluster);
        });
      });
    },
    toGeoJson(data) {
      ...
      for (let i = 0; i < data.length; i++) {
        _data.push({
          ...
          geometry: {
            type: 'Point',
            coordinates: [Number(data[i].Lon), Number(data[i].Lat)]
          }
        });
      }
      return _data;
    },
    getCluster(points, bbox, zoom) {
      const sc = new Supercluster({
        radius: 40,
        maxZoom: 16
      });
      sc.load(points);
      const cluster = sc.getClusters(bbox, zoom);
      return cluster;
    },
